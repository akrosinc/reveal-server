CREATE
MATERIALIZED VIEW IF NOT EXISTS location_metadata_double_aggregate
AS
...
from (
    SELECT obj ->>'tag' as tag, obj ->>'tagKey' as tag_key, cast (obj->'current'->'value'->>'valueDouble' as float) as value, lm.location_identifier as location_identifier, obj->'current'->'meta'->>'planId' as plan_identifier
    From location_metadata lm, jsonb_array_elements(lm.entity_value->'metadataObjs')
