...
FROM ( SELECT COALESCE(e.base_entity_identifier, e.location_identifier) AS location_identifier,
    ...
    WHERE (arr.obj ->> 'fieldCode'::text) !~~ '%AAAAAAAAAAAAA%'::text AND (arr.obj ->> 'fieldCode'::text) !~~ 'serial_number%'::text) eve
...
FROM ( SELECT eve_1.location_identifier,
       ...
       WHERE eve_1.isuuid = false AND eve_1.isjsonb = false AND eve_1.isdate = false) eve
         LEFT JOIN location l ON eve.ancestor = l.identifier
    WITH DATA;

CREATE INDEX event_aggregation_string_count_ancestor_idx
...
CREATE INDEX event_aggregation_numeric_ancestor_idx
