...
FROM plan_locations pl
         ...
         LEFT JOIN (SELECT arr.item_object AS parent,
                   ...
                   ON parents.location_identifier = l.identifier
         ...
         LEFT JOIN (SELECT pl_1.identifier                                                             AS location_id,
                           ...
                           count(*)
                           FILTER (WHERE l_1.location_property ->> 'surveyLocationType' is null OR
                                         l_1.location_property ? 'surveyLocationType' =
                                         false)                                                        as location_without_survey_location_type,
                           count(*)
                           FILTER (WHERE l_1.location_property ->> 'surveyLocationType' = 'waterbody') as water_body_count
                    FROM (SELECT arr.item_object AS parent,
                          ...
                          FROM location_relationship lr,
                             ...
                             LEFT JOIN location l_1
                                       ON l_1.identifier = parents_1.location_identifier
                             LEFT JOIN location pl_1 ON pl_1.identifier = parents_1.parent
                             LEFT JOIN geographic_level gl_1
                                       ON gl_1.identifier = l_1.geographic_level_identifier
                             LEFT JOIN geographic_level pgl_1
                                       ON pgl_1.identifier = pl_1.geographic_level_identifier
                    WHERE gl_1.name::text = 'structure'::text
                    GROUP BY pl_1.identifier, pl_1.name, pgl_1.name) structures(location_id, name,
                                                                                name_1,
                                                                                structure_count,
                                                                                location_without_survey_location_type,
                                                                                water_body_count)
                   ON structures.location_id = l.identifier
WHERE gl.name::text =
      lh.node_order[array_position(lh.node_order, 'structure'::character varying) - 1]::text;

CREATE UNIQUE INDEX ON assigned_structure_counts (plan_identifier, parent_location_identifier,
                                                  location_identifier);