...
public class MetaDataJdbcService {
  ...
  public Map<String,Pair<Class<?>,Object>> getMetadataFor(String entityTable, UUID identifier){
    ...
    String query = "SELECT t.key,t.value->>'value',t.value->>'entity_tag',et.value_type From (\n"
        ...
        + "inner join "+entityTable+"_metadata lm on l.identifier = lm."+entityTable+"_identifier\n"
    ...
    return jdbcTemplate.query(query ,(rs, rowNum) ->
        ...
        mapOfKeyAndPair -> mapOfKeyAndPair.entrySet().stream()).collect(Collectors.toMap(Entry::getKey,Entry::getValue));

  }


  private Map<String, Pair<Class<?>, Object>> getMapFromResultSet(ResultSet rs) throws SQLException {
    ...
    switch(valueType){
      case "integer": pair = Pair.of(Integer.class, rs.getInt(2));
        break;
      case "string": pair =Pair.of(String.class, rs.getString(2));
        break;
      case "boolean":pair = Pair.of(Boolean.class, rs.getBoolean(2));
        break;
    }
