...
public static Task entityFromRequestObj(TaskCreateRequest taskRequest, Action action, LookupTaskStatus lookupTaskStatus) {
  return Task.builder()
      ...
      .executionPeriodEnd(Date.from(
          taskRequest.getExecutionPeriodStart().atStartOfDay(ZoneId.systemDefault()).toInstant())).build();
}
}
