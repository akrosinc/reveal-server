...
public class OrganizationResponseFactory {
    ...
    public static OrganizationResponse fromEntityWithMembers(Organization organization) {
        List<UserResponse> users = !organization.getUsers().isEmpty() ? organization.getUsers().stream().map(UserResponseFactory::fromEntityWithoutOrganizations).collect(Collectors.toList()) : Collections.emptyList();
        return OrganizationResponse.builder()
                .identifier(organization.getIdentifier())
                .name(organization.getName())
                .active(organization.isActive())
