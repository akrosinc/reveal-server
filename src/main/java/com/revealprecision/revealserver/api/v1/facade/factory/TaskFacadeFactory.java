...
public class TaskFacadeFactory {
  ...
  public static TaskFacade getEntity(Task task,String businessStatus,String userName,String group){

    String entity;
    if (task.getAction().getLookupEntityType().getCode().equals("Person")){
       entity = task.getPerson().getIdentifier().toString();
    }
    else if (task.getAction().getLookupEntityType().getCode().equals("Location")){
    ...
    return TaskFacade.builder()
        ...
        .authoredOn(DateTimeFormatter.getDateTimeFacadeStringFromLocalDateTime (task.getAuthoredOn()))
        .description(task.getAction().getDescription())
        .executionPeriod(Period
            .between(DateTimeFormatter.getDateTimeFacadeStringFromLocalDateTime (task.getExecutionPeriodStart().atStartOfDay())
                ,DateTimeFormatter.getDateTimeFacadeStringFromLocalDateTime (task.getExecutionPeriodEnd().atStartOfDay())))
        ...
        .lastModified(DateTimeFormatter.getDateTimeFacadeStringFromLocalDateTime (task.getLastModified()))
