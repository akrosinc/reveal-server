...
public interface LocationRelationshipRepository extends JpaRepository<LocationRelationship, UUID> {
    ...
    @Query(value =
        "SELECT cast(lr.identifier as varchar) identifier"
            ...
            + "WHERE gl.name != 'structure' ", nativeQuery = true)
    List<LocationRelationshipProjection> findLocationTreeWithoutStructures(
        UUID locationHierarchyIdentifier);
    ...
    List<LocationChildrenCountProjection> getLocationChildrenCount(
        UUID locationHierarchyIdentifier);
    ...
    List<LocationRelationship> findLocationRelationshipsByLocation_IdentifierAndLocationHierarchy_Identifier(
        UUID locationIdentifier, UUID hierarchyIdentifier);
    ...
    List<LocationStructureCount> getNumberOfStructures(UUID hierarchyIdentifier,
    ...
    @Query(value =
        "select cast(lr.location_identifier as varchar) as identifier, ( select count(*) from location_relationship lre "
            ...
            + "left join geographic_level gl on gl.identifier = l.geographic_level_identifier "
            + "where gl.name = :geographicLevel and lr.location_hierarchy_identifier = :hierarchyIdentifier", nativeQuery = true)
    List<LocationStructureCount> getNumberOfStructuresBasedOnParentId(UUID hierarchyIdentifier,
        String geographicLevel);
    ...
    List<LocationRelationshipAncestryProjection> getLocationAncestryListsFromLocationIds(
        List<UUID> ids);
    ...
    LocationAndHigherParentProjection getHigherLocationParentByLocationAndParentGeographicLevelType(
        UUID locationIdentifier, UUID locationHierarchyIdentifier,
        String parentGeographicLevelName);

    @Query(
        "SELECT lr.identifier as identifier,l.name as  locationName,l.identifier as locationIdentifier , "
